import { findAll, findById, create } from '../src/services/charge.service';
import { db } from '../src/db';
import { chargesTable } from '../src/db/schema';
import { and, eq, sql } from 'drizzle-orm';
import { getYearTimestampRange } from '../src/utils/date.utils';
...
describe('Charge Service', () => {
  ...
  describe('findAll', () => {
    ...
    it('should filter charges by account ID', async () => {
      ...
      const result = await findAll(userId, accountId);
    ...
    it('should filter charges by year', async () => {
      ...
      const year = 2024;
      const mockCharges = [
        { id: 1, userId, created: 1704067200 }, // Jan 1, 2024
      ];
      ...
      const result = await findAll(userId, undefined, year);
    ...
    it('should use UTC timestamps for year filtering', async () => {
      ...
      await findAll(userId, undefined, year);

      // Verify year range is calculated correctly
    ...
    it('should filter by both accountId and year', async () => {
      ...
      const result = await findAll(userId, accountId, year);
  ...
  describe('findById', () => {
    it('should find a charge by ID and userID', async () => {
      ...
      const result = await findById(chargeId, userId);
    ...
    it('should verify user ownership', async () => {
      ...
      await findById(chargeId, userId);
  ...
  describe('create', () => {
    it('should create a new charge', async () => {
      ...
      const result = await create(newCharge);
  ...
  describe('Error Handling', () => {
    it('should handle database errors in findAll', async () => {
      ...
      await expect(findAll(userId)).rejects.toThrow('Database error');
    });
    ...
    it('should handle database errors in findById', async () => {
      ...
      await expect(findById(chargeId, userId)).rejects.toThrow('Database error');
    });
    ...
    it('should handle database errors in create', async () => {
      ...
      await expect(create(newCharge)).rejects.toThrow('Database error');
    });
  });
